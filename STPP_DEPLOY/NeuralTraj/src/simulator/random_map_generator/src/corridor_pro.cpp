...
{        
	...
	int startid, endid;
    nh.getParam("start_id", startid);
	nh.getParam("end_id", endid);
    ...
    std::cout << "startid: " << startid << " endid: "<<endid << std::endl;
    {   
        for(int i = startid; i <= endid; i++){
            double flatObcs[7200] = {0};
            std::ifstream fin(std::string("/home/han/2023codes/data/obcs/obc")+std::to_string(i)+ std::string(".dat"), std::ios::binary);
            ...
            for(int j = 1; j <=400; j++){
                double path[610] = {0};
                std::ifstream fin(std::string("/home/han/2023codes/data2/e")+std::to_string(i)+std::string("/path")+std::to_string(j) +std::string(".dat"), std::ios::binary);
                fin.read((char*)path, sizeof(double) * 610);
                fin.close();
                std::vector<Eigen::Vector3d> statelist;
                std::vector<cv::Mat> channels_all;
                cv::Mat merge_img;
                for(int k = 0; k < 100; k++){
                    double px = path[10 + k*6+1];
                    double py = path[10 + k*6+2];
                    double yaw = path[10 + k*6+3];
                    Eigen::Vector3d state; state << px, py, yaw;
                    cv::Mat corImage(dimx, dimy, CV_8UC1);
                    corImage =cv::Mat::zeros(dimx, dimy, CV_8UC1);
                    Eigen::MatrixXd rec = plan_utils::getDilateRec(state, gridmap);
                    for (int ix = 0; ix < dimx; ix++) {
                        for (int jy = 0; jy < dimy; jy++) {
                            Eigen::Vector2d pos = gridmap.intToFloat(Eigen::Vector2i(ix, jy));
                            if(gridmap.isOccupied(pos)){
                                continue;
                            }
                            if(plan_utils::ifInside(pos, rec)){
                                corImage.at<uchar>(ix,jy)=1;
                            }

                        }
                    }
                    statelist.push_back(state);
                    channels_all.push_back(corImage);
                }
                cv::merge(channels_all, merge_img);
                // std::cout << merge_img.rows << " " << merge_img.cols << " "<< merge_img.channels() << std::endl;
                cv::FileStorage fs(std::string("/home/han/2023codes/data2/e")+std::to_string(i)+string("/cor")+std::to_string(j)+string(".xml"), cv::FileStorage::WRITE);
                fs << "instance" << merge_img;
                fs.release();
                std::cout << "mapid: "<< i << " pathid: "<<j<< std::endl;
    ...
    ROS_WARN_STREAM("FINISH! startid, endid:  "<< startid <<" "<<endid );
