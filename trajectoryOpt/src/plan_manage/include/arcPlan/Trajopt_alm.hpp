    ...
    public:
            ...
            std::shared_ptr<visualization::Visualization> vis_tool,std::string name = "nn", std::vector<Eigen::VectorXd> ds_container = {})
        {
            ...
            for(int i = 0; i < trajnum; i++) {
                ...
                neqc_num += piecenums[i] * (traj_res+1) * 6; //  vel>0.1 cur  s' >= 0   vel acc  omega
...
// 
            ...
            //     }
            //     ROS_WARN_STREAM("esdf: "<<debugEsdf);
                    ...
                    //full shape model
                    {
                        for(const Eigen::Vector2d resPt : conpts){
                            ...
                            */
                            ...
                            double vioSdist = (-dis + safeMargin+0.05);//hzchzchzc
                            if(vioSdist > 0){
                                double pena, penaD;
        ...
        UgvTrajectory getOptTraj(){
            ...
            ROS_WARN_STREAM("proposed max cur: "<<getMaxCur());
            // ROS_WARN_STREAM("dptraj max cur dot: "<<getMaxCurDot());
            ROS_WARN_STREAM("proposed max omega: "<<getMaxOmega());
            return optTraj;
        }
        void debugVis(){
            if(isstart&&isdebug_){
